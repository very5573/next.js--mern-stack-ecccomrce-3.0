...
export const updateOrder = async (req, res) => {
  ...
  try {
    ...
    for (const orderId of orderIds) {
      ...
      while (currentStatus !== newStatus) {
        ...
        if (!nextStatuses.includes(newStatus)) {
          if (nextStatuses.length === 0) {
            invalidOrders.push({ orderId: order._id, reason: "Invalid transition" });
            break;
          }
      ...
      if (invalidOrders.find((i) => i.orderId.toString() === order._id.toString())) {
        continue; // skip invalid transition
      }

      // Process each status in path
      for (const status of path) {
        if (status === "Cancelled" && order.orderStatus !== "Cancelled" && order.orderStatus !== "Delivered") {
...
export const getAllOrders = async (req, res) => {
  try {
    ...
    // 2️⃣ Fetch orders + total count together (Fast)
    ...
    // 3️⃣ Format orders for frontend
    const formattedOrders = orders.map(order => ({
      _id: order._id,
      user: order.user,
      shippingInfo: order.shippingInfo,
